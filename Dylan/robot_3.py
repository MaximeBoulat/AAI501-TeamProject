...
def get_action(from_pos, to_pos):
    ...
    for i, (ddx, ddy) in enumerate(DIRECTIONS):
        if (dx == ddx and dy == ddy):
            print(f"Output direction {DIRECTIONS[i]} {dx},{dy}")
            return i
    return None
...
def get_goal_direction_radians(current, goal):
    ...
    #TODO test that this angle is indeed correct
...
#            # --- NEW: Compute action_direction in radians ---
#            dx, dy = DIRECTIONS[action][1], -DIRECTIONS[action][0]  # x, -y
#            action_direction = math.atan2(dx, dy) % (2 * math.pi)
